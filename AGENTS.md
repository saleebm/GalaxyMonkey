...
This is a two-platform monorepo: **ios/** (shipping SpriteKit game) and
**android/** (LibGDX/Kotlin port, in progress).

### ios/ — SpriteKit game (do not break)

The iOS Swift source under `ios/GalaxyMonkey/` is the **behavioral spec** the
Android port mirrors. Gate changes via:

```bash
cd ios && ./run-sim.sh        # build + launch on Simulator
cd ios && ./test-smoke.sh     # full xcodebuild test suite
```

### android/ — LibGDX/Kotlin Android port

Gradle multi-module: `:core` (Kotlin JVM game logic) + `:app` (Android shell).
Package `dev.copt.galaxymonkey`, landscape-only, fullscreen.

```bash
cd android && ./gradlew :app:assembleDebug         # build APK
cd android && ./gradlew :core:test                  # headless unit tests
cd android && ./gradlew :app:connectedDebugAndroidTest  # instrumentation (needs device)
```

### SpriteKit → LibGDX mapping

| iOS (SpriteKit)           | Android (LibGDX)                |
|---------------------------|---------------------------------|
| GalaxyMonkeyApp.swift     | AndroidLauncher.kt              |
| GameScene.swift           | GalaxyMonkeyGame.kt + GameScreen.kt |
| Info.plist orientation    | AndroidManifest sensorLandscape |
| SKSpriteNode              | SpriteBatch + TextureRegion     |
| SKAction animations       | AnimationCatalog (LibGDX Animation) |
| touchesBegan/Moved/Ended  | InputProcessor / InputMultiplexer |
| SKPhysicsContactDelegate  | CollisionSystem (circle overlap) |
