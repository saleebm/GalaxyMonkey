#!/usr/bin/env bash
# Upload build/export/GalaxyMonkey.ipa to App Store Connect via altool.
#
# Required env vars:
...
# drag build/export/GalaxyMonkey.ipa into it. Transporter uses Xcode's
# signed-in account directly.

set -euo pipefail

PROJECT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
IPA="$PROJECT_DIR/build/export/GalaxyMonkey.ipa"
